...
// что-то пошло не так в OAuth-колбэке
const oauthError = computed(() => route.query.error === '1')

function signInWithGitHub() {
  loading.value = true
  // Полный переход на серверный OAuth-роут (nuxt-auth-utils)
...
const stack = ['Nuxt 4', 'PostgreSQL', 'n8n', 'OpenAI', 'Vercel']
</script>
  ...
  <div class="min-h-screen bg-background flex flex-col items-center justify-center px-4 py-16">
    ...
    <div class="relative w-full max-w-sm space-y-8">
      ...
      <div class="space-y-3">
        ...
        <p v-if="oauthError" class="text-xs text-destructive text-center">
