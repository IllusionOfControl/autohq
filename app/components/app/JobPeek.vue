...
watch(
  ...
  async ([isOpen, id]) => {
    ...
    try {
      details.value = await $fetch<JobDetails>(`/api/jobs/${id}`)
    } catch {
      details.value = null
    } finally {
...
async function setStatus(s: JobStatus) {
  ...
  try {
    await $fetch(`/api/jobs/${job.id}`, { method: 'PATCH', body: patch })
    emit('updated', job.id, patch)
  } finally {
...
async function del() {
  ...
  try {
    ...
    alert('Delete failed: ' + (e?.data?.message ?? e?.message ?? 'unknown'))
  } finally {
